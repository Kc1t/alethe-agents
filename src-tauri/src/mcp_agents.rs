...
use toml_edit::{value, Array, DocumentMut, InlineTable, Item, Table, TableLike, Value as TomlValue};

use crate::mcp_model::{
    EnvEntry, EnvMap, McpAgent, McpScope, McpServer, McpTimeouts, McpTransport,
};
...
pub trait McpAdapter: Send + Sync {
    fn config_path(&self, scope: McpScope, repo: Option<&Path>) -> Option<PathBuf>;
    fn parse(&self, raw: &str) -> Result<Vec<McpServer>, String>;
    fn upsert(&self, raw: &str, server: &McpServer) -> Result<String, String>;
    fn remove(&self, raw: &str, name: &str) -> Result<String, String>;
    fn set_enabled(&self, raw: &str, name: &str, on: bool) -> Result<String, String>;
}
...
impl McpAdapter for ClaudeAdapter {
    fn config_path(&self, scope: McpScope, repo: Option<&Path>) -> Option<PathBuf> {
        match scope {
            McpScope::Global => mcp_home(&[".claude.json"]),
            McpScope::Project => repo.map(|root| root.join(".mcp.json")),
        }
    }

    fn parse(&self, raw: &str) -> Result<Vec<McpServer>, String> {
        parse_json_servers(raw, "mcpServers", false)
    }

    fn upsert(&self, raw: &str, server: &McpServer) -> Result<String, String> {
        json_upsert(raw, "mcpServers", CLAUDE_MANAGED, false, server)
    }

    fn remove(&self, raw: &str, name: &str) -> Result<String, String> {
        json_remove(raw, "mcpServers", name)
    }

    fn set_enabled(&self, _raw: &str, _name: &str, _on: bool) -> Result<String, String> {
        Err("unsupported_disable".to_string())
    }
}

impl McpAdapter for AntigravityAdapter {
    fn config_path(&self, scope: McpScope, _repo: Option<&Path>) -> Option<PathBuf> {
        match scope {
...
impl McpAdapter for OpenCodeAdapter {
    fn config_path(&self, scope: McpScope, repo: Option<&Path>) -> Option<PathBuf> {
        let dir = match scope {
            ...
            McpScope::Project => repo?.to_path_buf(),
        };
    ...
    fn upsert(&self, raw: &str, server: &McpServer) -> Result<String, String> {
        json_upsert(raw, "mcp", OPENCODE_MANAGED, true, server)
    }

    fn remove(&self, raw: &str, name: &str) -> Result<String, String> {
        json_remove(raw, "mcp", name)
    }
...
impl McpAdapter for CodexAdapter {
    fn config_path(&self, scope: McpScope, repo: Option<&Path>) -> Option<PathBuf> {
        match scope {
            ...
            McpScope::Project => repo.map(|root| root.join(".codex").join("config.toml")),
        }
    }
...
fn parse_json_servers(
    ...
    let Some(map) = value.get(root_key).and_then(Value::as_object) else {
        return Ok(Vec::new());
    };
    let mut out: Vec<McpServer> = map
        .iter()
        .filter_map(|(name, entry)| entry.as_object().map(|obj| json_server(name, obj, interpolate)))
        .collect();
...
fn json_env(value: Option<&Value>, interpolate: bool) -> EnvMap {
    ...
    map.iter()
        .filter_map(|(key, raw)| raw.as_str().map(|text| (key.clone(), env_entry(text, interpolate))))
        .collect()
}
...
fn json_error(error: serde_json::Error) -> String {
    format!("unparsable:json line {} column {}", error.line(), error.column())
}
...
fn json_env_value(entry: &EnvEntry, interpolate: bool) -> Option<Value> {
    ...
    entry.literal.as_ref().map(|text| Value::String(text.clone()))
}
...
fn json_upsert(
    ...
    if !object.get(root_key).map(Value::is_object).unwrap_or(false) {
        object.insert(root_key.to_string(), Value::Object(Map::new()));
    }
    let servers = object
        .get_mut(root_key)
...
fn json_remove(raw: &str, root_key: &str, name: &str) -> Result<String, String> {
    let mut root = json_root(raw)?;
    let Some(servers) = root.get_mut(root_key).and_then(Value::as_object_mut) else {
...
"#;
    ...
    fn by_name<'a>(servers: &'a [McpServer], name: &str) -> &'a McpServer {
        servers.iter().find(|s| s.name == name).expect("server present")
    }
    ...
    fn codex_reads_every_table_shape() {
        ...
        assert!(matches!(&discord.transport, McpTransport::Stdio { command, .. } if command == "npx"));
        assert_eq!(
            discord.env.get("DISCORD_TOKEN").and_then(|e| e.literal.as_deref()),
            Some("a-live-secret-token-value")
        );

        let figma = by_name(&servers, "figma");
        assert!(matches!(&figma.transport, McpTransport::Http { url, .. } if url.contains("figma")));
    }
    ...
    fn claude_reads_stdio_and_http_entries() {
        ...
        let servers = ClaudeAdapter.parse(raw).expect("parses");
        assert_eq!(servers.len(), 2);
        let swarm = by_name(&servers, "swarm");
        assert_eq!(swarm.env.get("K").and_then(|e| e.literal.as_deref()), Some("V"));
    ...
    fn claude_honours_an_explicit_disabled_flag() {
        ...
        assert!(!ClaudeAdapter.parse(raw).expect("parses")[0].enabled);
    }
    ...
    fn opencode_splits_the_command_array_and_reads_passthrough() {
        ...
        let servers = OpenCodeAdapter.parse(raw).expect("parses");
        let swarm = &servers[0];
        ...
        assert!(!swarm.enabled);
        assert_eq!(
            swarm.env.get("QUADRANT").and_then(|e| e.passthrough_from.as_deref()),
            Some("QUADRANT")
        );
        assert_eq!(swarm.env.get("MODE").and_then(|e| e.literal.as_deref()), Some("prod"));
    }

    #[test]
    fn opencode_config_without_mcp_is_empty() {
        assert!(OpenCodeAdapter.parse(r#"{"model":"a/b"}"#).expect("parses").is_empty());
    }

    #[test]
    fn antigravity_shares_the_claude_shape() {
        let raw = r#"{"mcpServers":{"swarm":{"type":"stdio","command":"node","args":["a.js"]}}}"#;
        assert_eq!(AntigravityAdapter.parse(raw).expect("parses").len(), 1);
    }
    ...
    fn antigravity_has_no_project_scope() {
        ...
        assert!(AntigravityAdapter
            .config_path(McpScope::Project, Some(&repo))
            .is_none());
    }
    ...
    fn codex_upsert_leaves_every_other_table_untouched() {
        ...
        for server in &before {
            let kept = after.iter().find(|item| item.name == server.name).expect("kept");
    ...
    fn codex_upsert_round_trips_timeouts_and_passthrough() {
        ...
        server.env.insert("MODE".to_string(), EnvEntry::literal("prod"));
        server.env.insert("FOO".to_string(), EnvEntry::passthrough("FOO"));
        ...
        assert_eq!(written.env.get("MODE").and_then(|e| e.literal.as_deref()), Some("prod"));
        assert_eq!(
            written.env.get("FOO").and_then(|e| e.passthrough_from.as_deref()),
            Some("FOO")
        );
    }
    ...
    fn codex_upsert_keeps_a_hand_written_env_section_as_a_section() {
        ...
        server.env.insert("DISCORD_GUILD_ID".to_string(), EnvEntry::literal("456"));
    ...
    fn codex_mutations_report_not_found_for_an_unknown_server() {
        assert_eq!(codex_remove(CODEX_FIXTURE, "ghost").unwrap_err(), "not_found");
        assert_eq!(
  ...
  "projects": { "D:\\repo": { "allowedTools": [] } }
    ...
    fn claude_upsert_keeps_top_level_key_order_and_unrelated_keys() {
        let next = ClaudeAdapter
            .upsert(CLAUDE_FIXTURE, &probe("alethe-probe"))
            .expect("writes");
        ...
        let parsed = ClaudeAdapter.parse(&next).expect("parses");
    ...
    fn claude_upsert_preserves_unknown_keys_on_the_entry() {
        ...
        let next = ClaudeAdapter.upsert(raw, &probe("a")).expect("writes");
    ...
    fn claude_cannot_disable_a_server() {
        assert_eq!(
            ClaudeAdapter
                .set_enabled(CLAUDE_FIXTURE, "figma", false)
                .unwrap_err(),
            "unsupported_disable"
        );
    }
    ...
    fn claude_upsert_creates_the_root_key_in_an_empty_file() {
        let next = ClaudeAdapter.upsert("", &probe("a")).expect("writes");
        assert_eq!(ClaudeAdapter.parse(&next).expect("parses").len(), 1);
    }

    #[test]
    fn claude_remove_reports_not_found() {
        assert_eq!(
            ClaudeAdapter.remove(CLAUDE_FIXTURE, "ghost").unwrap_err(),
            "not_found"
        );
        let next = ClaudeAdapter.remove(CLAUDE_FIXTURE, "figma").expect("removes");
        assert!(ClaudeAdapter.parse(&next).expect("parses").is_empty());
  ...
  "mcp": {
    ...
    fn opencode_upsert_writes_one_command_array_and_interpolated_env() {
        let mut server = probe("alethe-probe");
        server.env.insert("FOO".to_string(), EnvEntry::passthrough("FOO"));
        let next = OpenCodeAdapter.upsert(OPENCODE_FIXTURE, &server).expect("writes");

        assert!(next.contains("\"$schema\""));
        assert!(next.contains("\"model\""));
        assert!(next.contains("{env:FOO}"));

        let parsed = OpenCodeAdapter.parse(&next).expect("parses");
        let written = by_name(&parsed, "alethe-probe");
    ...
    fn opencode_remove_keeps_every_sibling_key() {
        let next = OpenCodeAdapter.remove(OPENCODE_FIXTURE, "swarm").expect("removes");
        assert!(next.contains("\"$schema\""));
        assert!(next.contains("\"model\""));
        assert!(OpenCodeAdapter.parse(&next).expect("parses").is_empty());
    }

    #[test]
    fn opencode_set_enabled_writes_the_flag() {
        let next = OpenCodeAdapter
            .set_enabled(OPENCODE_FIXTURE, "swarm", false)
            .expect("writes");
        assert!(!OpenCodeAdapter.parse(&next).expect("parses")[0].enabled);
    }

    #[test]
    fn antigravity_writes_the_claude_shape() {
        let raw = r#"{"mcpServers":{}}"#;
        let next = AntigravityAdapter.upsert(raw, &probe("a")).expect("writes");
        assert_eq!(AntigravityAdapter.parse(&next).expect("parses").len(), 1);
    }
