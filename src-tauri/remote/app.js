...
function connectSocket() {
  ...
  socket.onopen = () => { setConnectionState('live'); socketAuthenticated = false; socket.send(JSON.stringify({ type: 'hello', token, deviceName: /Android|iPhone|iPad/i.test(navigator.userAgent) ? 'Mobile device' : 'Browser' })) }
...
async function boot() {
  if (!token) { app.innerHTML = '<div class="fatal"><div class="fatal-card"><span class="brand-mark">›_</span><strong>Pairing link required</strong><p>Scan the QR code shown in Alethe to connect this device.</p></div></div>'; return }
  ...
  catch (error) { app.innerHTML = `<div class="fatal"><div class="fatal-card"><span class="brand-mark">!</span><strong>Connection unavailable</strong><p>${escapeHtml(error.message || error)}</p></div></div>` }
}
