...
export async function watchAndPersistDiscoveredSession(
  ...
  const { agent, cwd, sessionPersistenceKey, spawnedPtyId, discoveredSessionsBeforePromise, isCancelled, onSessionId } =
    opts
  ...
  while (!isCancelled()) {
    ...
    const newSession = claimDiscoveredSession(agent, cwd, before, filteredSessions, spawnedPtyId)
    if (newSession) {
