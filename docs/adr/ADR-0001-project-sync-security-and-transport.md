...
1. **Separate identity, device trust, authorization, and transport.** OIDC identifies an account. A per-installation asymmetric key identifies a device. A versioned grant authorizes a subject to one random project identity. A transport only carries authenticated protocol messages.
