...
- **Contact info panel**: clicking a direct chat's header (avatar, name, connection badge) now opens a right-side panel with the contact's details and the actions that used to be three small icon buttons crowded into the sidebar contact list row (rename, remove, delete conversation + history) — one click closer to the conversation they act on, instead of easy-to-misread icons next to every contact in the list.
