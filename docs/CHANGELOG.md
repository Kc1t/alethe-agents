...
- **Fixed the commit graph drawing wrong paths**: bounding the graph's width by dropping or merging deep lanes corrupted the drawing — merging stacked unrelated strands at one position, and dropping left strands that appeared and vanished with no connection, putting commits on a line that wasn't theirs and curves that led nowhere. The width is now bounded by scaling the lanes (and dots and stroke widths) down to fit instead, so every lane is always drawn in its own column and a dense history simply renders tighter.
