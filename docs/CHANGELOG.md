...
- **Fixed graph lanes merging into one smeared line**: lanes beyond the drawn limit were being collapsed onto the last column, stacking several unrelated strands at the same position. They are now omitted instead, so the lanes that are drawn stay visually distinct.
