...
- Tasks now show as a Kanban-style board with three fixed columns (Todas / Abertas / Concluídas) instead of a single vertical list with filter buttons; dragging a task card between the "Abertas" and "Concluídas" columns completes or reopens it, replacing the earlier text-only complete/reopen button as the primary way to move a task.
