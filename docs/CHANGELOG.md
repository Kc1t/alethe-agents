...
- **Fixed inconsistent avatar fallback in chat message bubbles**: with no real photo configured, your own messages showed the app's generic default icon while the other person's messages fell back to their bare initial instead (or nothing at all, before the contact's thumbnail synced) — two different fallbacks for the same "no photo set" case, which read as "their avatar synced, mine didn't" even when neither side had set one. Both now consistently fall back to the app's default icon.
